...
next_req_id = 1
pending_api_requests = {} # Stores req_id: request_type_string
...
def calculate_indicators(prices_list: list[float]):
    ...
    # Ensure prices_series is a pd.Series before RSI delta calculation
    if not isinstance(prices_series, pd.Series):
        logging.warning(f"prices_series was not a pd.Series ({type(prices_series)}) before RSI delta calculation. Re-converting.")
        prices_series = pd.Series(prices_series, dtype=float) # Ensure it's a Series
    
    # RSI Calculation (original logic using prices_series.diff() within the apply)
    # The .diff() is inside the lambda, so the check above is a general safeguard for the prices_series object itself.
    # A more direct check before a standalone delta = prices_series.diff() would be:
    # delta = prices_series.diff()
    # gain = (delta.where(delta > 0, 0)).rolling(window=14).mean()
    # loss = (-delta.where(delta < 0, 0)).rolling(window=14).mean()
    # rs = gain / (loss + 1e-9) # Add epsilon to prevent division by zero
    # results['rsi'] = (100 - (100 / (1 + rs))).fillna(50).tolist()
    # Sticking to the original compact RSI calculation method for now:
...
def request_ohlcv_data(ws_app_instance, symbol_to_fetch, granularity_s, candle_count=100):
    ...
    if ws_app_instance and ws_app_instance.sock and ws_app_instance.sock.connected:
        ...
        next_req_id += 1
        pending_api_requests[current_req_id] = 'ohlcv_chart_data'
...
def request_daily_data(ws_app_instance, symbol_to_fetch):
    ...
    if ws_app_instance and ws_app_instance.sock and ws_app_instance.sock.connected:
        current_req_id = next_req_id
        next_req_id += 1
        pending_api_requests[current_req_id] = 'daily_summary_data'
...
def on_message_for_deriv(ws, message):
    ...
    if data.get('error'):
        failed_req_type = 'Unknown Request'
        if req_id_of_message is not None and req_id_of_message in pending_api_requests:
            failed_req_type = pending_api_requests.pop(req_id_of_message)
        logging.error(f"Deriv API Error for req_id {req_id_of_message} (type: {failed_req_type}): {data['error']}. Full message: {message}")
        return # Stop further processing of this error message

    msg_type = data.get('msg_type')
    # Note: req_id_received is the same as req_id_of_message if echo_req.req_id exists.
    # We will use req_id_of_message when we need the req_id from the echo_req.

    if msg_type == 'authorize':
        # Authorization responses don't typically have errors handled by the general error handler above
        # unless the authorize request itself had a req_id and failed in a specific way.
        # The 'error' field within the 'authorize' msg_type is for auth success/failure.
        if data.get('error'): # This is for auth specific error, not a general message error
        ...
        # Errors in subscribe response are usually within data['error'], handled by general error handler if req_id was used.
        # If no req_id, then this specific check is fine.
        if data.get('error'): # This specific error check might be redundant if subscribe always echoes req_id
        ...
        if req_id_of_message is not None:
            request_type = pending_api_requests.pop(req_id_of_message, None)
        
        logging.info(f"Received 'candles' data (req_id: {req_id_of_message}, type: {request_type}): {str(message)[:300]}...")
        logging.debug(f"Received {len(raw_candles_list)} raw candle objects for req_id {req_id_of_message} (type: {request_type}).")

        if request_type == 'ohlcv_chart_data':
            ...
            if raw_candles_list:
                ...
                logging.debug(f"Parsed {len(parsed_ohlcv_candles)} candles for OHLCV (req_id: {req_id_of_message}, type: {request_type}). First 1-2: {parsed_ohlcv_candles[:2] if parsed_ohlcv_candles else []}")
                with market_data_lock:
                    market_data['ohlcv_candles'] = parsed_ohlcv_candles
                logging.info(f"Updated market_data['ohlcv_candles'] with {len(parsed_ohlcv_candles)} candles (req_id: {req_id_of_message}, type: {request_type}).")
                if current_chart_type == 'ohlcv':
                    logging.info(f"OHLCV data received (req_id: {req_id_of_message}, type: {request_type}), recalculating indicators.")
                    ...
                    if prices_for_recalc:
                        ...
                        logging.info(f"Indicators recalculated using new OHLCV data (req_id: {req_id_of_message}, type: {request_type}).")
            else:
                logging.warning(f"No candle data in 'ohlcv_chart_data' response (req_id: {req_id_of_message}).")

        elif request_type == 'daily_summary_data':
            if raw_candles_list:
                ...
                if isinstance(candle, dict):
                    ...
                    logging.warning(f"Daily candle data item not in dict format (req_id: {req_id_of_message}, type: {request_type}).")
            else: 
                logging.warning(f"No candle data in 'daily_summary_data' response (req_id: {req_id_of_message}).")
        else:
            logging.warning(f"Received 'candles' message with unexpected or untracked req_id: {req_id_of_message} (type: {request_type}). Message: {str(message)[:300]}")
            
    # The general error handler at the top should catch most errors with req_id.
    # This specific check might be redundant if all error messages include echo_req.
    # elif data.get('error'): 
    #     logging.error(f"Deriv API Error (unhandled by specific req_id logic): {data['error']}. Full message: {message}")
